...
def commenter_publications(bot, username, commentaire, nombre1):
    user_id = bot.get_user_id_from_username(username)
    # Récupérer toutes les publications de l'utilisateur
    publications = bot.get_user_medias(user_id)
    for publication in publications:
        media_id = publication['pk']
        # Poster un commentaire sur la publication
        bot.comment(media_id, commentaire)
        nombre1 += 1
...
def commenter_reels(bot, username, commentaire, nombre2):
    user_id = bot.get_user_id_from_username(username)
    # Récupérer tous les reels de l'utilisateur
    reels = bot.get_user_reels(user_id)  # Utilisation de la méthode get_user_reels
    for reel in reels:
        reel_id = reel['id']
        # Poster un commentaire sur le reel
        bot.comment(reel_id, commentaire)
        nombre2 += 1
...
if choix2 == "1":
    ...
    while True:
        ...
        try:
            ...
            liste_commentaire = json.loads(liste_json)
            break
    ...
    # Initialisation du bot Instabot
    bot = Bot()
    ...
    commenter_publications(bot, profile_username, commentaire)
    commenter_reels(bot, profile_username, commentaire)
    ...
    # Appeler la méthode login avec un seul argument (un dictionnaire)
    ...
    # Commenter toutes les publications et reels du profil
    commenter_publications(bot, profile_username, commentaire, nombre1)
    commenter_reels(bot, profile_username, commentaire, nombre2)
